...
可选 ``LLM_KEY_ROTATION_MAX_ATTEMPTS``：同一轮用户消息下最多尝试多少次 POST
（**线性**逐槽切换，耗尽即止）；默认 ``密钥槽位数``（每槽最多一次）；仅 1 个密钥时为 ``1``。
...
def _should_rotate_on_transport_error(exc: BaseException) -> bool:
    """
    连接/读阶段失败（含超时、对端无完整响应即断开）：可换下一密钥重新建连。
    明确排除 SSL 证书校验类错误（换密钥通常无效）。
    """
    if isinstance(exc, urllib.error.HTTPError):
        return False
    s = str(exc).lower()
    if "certificate verify failed" in s or "sslcertverificationerror" in s:
        return False
    return True


def _llm_key_rotation_max_attempts(n_keys: int) -> int:
    """
    线性轮询的最大 POST 次数：每个密钥槽至多尝试一次，不做环形重试。
    环境变量 ``LLM_KEY_ROTATION_MAX_ATTEMPTS``：正整数；未设置时默认 ``n_keys``（逐个换，不回头）；``n<=1`` 时为 ``1``。
    """
    if n_keys <= 1:
        return 1
    raw = _env_first("LLM_KEY_ROTATION_MAX_ATTEMPTS")
    if raw is not None:
        try:
            v = int(str(raw).strip())
            return max(n_keys, v)
...
def _post_chat_completion_once(
    ...
    *,
    ...
    _LOG.info(
        "[环节:HTTP] 即将进入阻塞等待：POST 密钥槽 %s/%s%s，JSON payload=%s 字节，读超时=%.1fs，URL=%s",
        key_attempt,
        key_total,
        (
            f"，总尝试 {global_attempt}/{global_max}"
            if global_attempt is not None and global_max is not None
            else ""
        ),
        len(data),
        timeout_sec,
...
def call_openai_compatible_chat(cfg: LlmEnvConfig, user_text: str) -> str:
    ...
    遇限流、连接超时或无完整 HTTP 响应时，在**多个密钥槽**之间**线性**逐槽切换重试（耗尽即止）；单密钥不循环。
    最大尝试次数见环境变量 ``LLM_KEY_ROTATION_MAX_ATTEMPTS``（默认 ``密钥槽位数``）。
    """
    ...
    if n > 1:
        _LOG.info(
            "[环节:API请求] POST %s model=%s 超时=%ss 单条user字符数=%s 密钥槽=%s 线性逐槽最多=%s 次",
            url,
            cfg.model,
            cfg.timeout_sec,
            len(combined),
    ...
    # 当前要 POST 的密钥槽下标（0=主密钥）；失败后顺序移至下一槽，耗尽即止
    key_slot_cursor = 0
    attempt = 0
    while attempt < max_attempts:
        ...
        try:
            text = _post_chat_completion_once(
                ...
                token,
                cfg.timeout_sec,
                key_attempt=display_slot,
                key_total=n,
                global_attempt=attempt,
                global_max=max_attempts,
            )
            ...
            if _should_rotate_llm_api_key(e.code, detail) and n > 1 and attempt < max_attempts:
                ...
                _LOG.warning(
                    "[环节:密钥轮换] HTTP %s，%s 后换用密钥槽 %s/%s（第 %s/%s 次 POST，线性换槽）URL=%s 响应片段=%s",
                    e.code,
            ...
            if n <= 1 or attempt >= max_attempts or not _should_rotate_on_transport_error(e):
                ...
                if attempt >= max_attempts and n > 1:
                    raise RuntimeError(
                        f"大模型请求已达最大尝试次数 {max_attempts}（{n} 个密钥槽线性轮询）仍未成功；"
                        f"可检查网络/上游或增加可用密钥槽。"
                    ) from e
            ...
            nxt = (key_slot_cursor + 1) % n
            _LOG.warning(
                "[环节:密钥轮换] 连接/读超时或无完整响应：%s；%s 后换用密钥槽 %s/%s（第 %s/%s 次 POST，线性换槽）",
                e,
                f"休眠 {backoff}s" if backoff > 0 else "无休眠",
                nxt + 1,
                n,
