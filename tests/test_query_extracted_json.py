...
def test_fill_template_content_and_visual_summary() -> None:
    document = {
        ...
        "kids": [
            {"type": "heading", "page number": 1, "content": "立案登记表"},
            {
                "type": "table",
                "page number": 1,
                "rows": [
    ...
    assert fields["案件来源"]["内容"] == "案件来源 行政检查中发现"
    assert (
        fields["经办机构负责人意见"]["内容"]
        == "经办机构负责人意见：建议立案。；视觉识别摘要：存在1个人的手写签名。"
    )
...
def test_level1_slash_either_alternative_fills_fields() -> None:
    ...
    result = _fill_template_for_document(document=document, template=template)
    assert result["立案登记表/登记表副本"]["字段"]["案件来源"]["内容"] == "案件来源 移送"
